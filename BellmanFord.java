...
public class BellmanFord {
	...
	BellmanFord(WGraph g, int source) throws BellmanFordException {
		 ...
		 * Integer.MAX_VALUE
		 * 
		 * When throwing an exception, choose an appropriate one from the ones given
		 * above
		 */
	...
	public int[] shortestPath(int destination) throws BellmanFordException {
		 ...
		 * the input destination If not path exists an Exception is thrown Choose
		 * appropriate Exception from the ones given
		 */
		...
		while (destination != 0) {
			s += (int) destination;
			destination = this.predecessors[destination];
		}
		s += 0;
		int[] path = new int[s.length()];

		for (int i = 0; i < s.length(); i++) {
			path[i] = s.charAt(s.length() - 1 - i) - '0';

			if (this.distances[path[i]] == Integer.MAX_VALUE) {
				throw new PathDoesNotExistException("Path to that destination is not possible.");
			}
		}
